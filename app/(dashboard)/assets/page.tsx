...
export default async function AssetsPage() {
  const assets = await db.asset.findMany({
    include: { category: true, company: true, assignments: { where: { returnedAt: null }, include: { employee: true } } },
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="rounded-lg border overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {assets.length === 0 && (
              <tr><td colSpan={10} className="text-center py-8 text-muted-foreground">No assets yet.</td></tr>
            )}
            {assets.map((asset) => {
