...
export default async function AssignmentsPage() {
  const [assets, employees, assignments] = await Promise.all([
    ...
    db.assetAssignment.findMany({
      ...
      include: { asset: { include: { category: true } }, employee: true, assignedByUser: true },
    }),
  ]);
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {active.length === 0 && (
                <tr><td colSpan={5} className="text-center py-8 text-muted-foreground">No active assignments.</td></tr>
              )}
              {active.map((a) => (
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {returned.length === 0 && (
                <tr><td colSpan={4} className="text-center py-8 text-muted-foreground">No returns yet.</td></tr>
              )}
              {returned.map((a) => (
